...
(function (NightingaleCompiler) {
    class Lexer {
        ...
        */
         ...
         */
        _current_position = 0, errors_stream = [], warnings_stream = [], isInString = false, isInComment = false, missingEndOfProgram = true, 
        /**
         ...
         *
        * @param {string} sourceCode source code from CodeMirror input field.
         */
        main(sourceCode) {
            this.output.push(new NightingaleCompiler.OutputConsoleMessage(LEXER, INFO, `Lexing program ${this.programNumber}...`));
            // The current substring of the source code that will be tested against tokens.
            var current_potential_lexeme = "???";
            LoopThroughSourceCodeWhileLoop: while (this._last_position <= sourceCode.length && this._current_position <= sourceCode.length) {
                current_potential_lexeme = sourceCode.substring(this._last_position, this._current_position);
                ...
                if (this.isInComment) {
                    ...
                    if (/(\*\/$)/.test(current_potential_lexeme)) {
                        ...
                        // Generate token
                        this._temp_token = new NightingaleCompiler.LexicalToken(END_BLOCK_COMMENT, /^(\*\/)$/, current_potential_lexeme, this._code_editor_line_number, this._code_editor_line_position + (current_potential_lexeme.length - offset));
                        this.emit_token_to_stream(this._temp_token);
                        this._code_editor_line_position += current_potential_lexeme.length - offset;
                ...
                // In string
                    ...
                    if (/^[a-z]$/.test(current_potential_lexeme) || RegExp("^ $").test(current_potential_lexeme) || RegExp("^\t$").test(current_potential_lexeme)) {
                        // Generate token
                        this._temp_token = new NightingaleCompiler.LexicalToken(CHARACTER, /^(\*\/)$/, current_potential_lexeme, this._code_editor_line_number, this._code_editor_line_position + 1);
                        this.emit_token_to_stream(this._temp_token);
                        // Adance tab 4 spaces
                        if (RegExp("^\t$").test(current_potential_lexeme)) {
                            this._code_editor_line_position += this.calc_relative_length(current_potential_lexeme);
                        } // if
                        else {
                    ...
                    // Quotation found in string
                        ...
                        this.emit_token_to_stream(new NightingaleCompiler.LexicalToken("END_STRING_EXPRESSION", null, current_potential_lexeme, this._code_editor_line_number, this._code_editor_line_position + 1));
                    ...
                    // Not a valid character
                        ...
                        this.emit_token_to_stream(new NightingaleCompiler.LexicalToken(INVALID_TOKEN, null, current_potential_lexeme, this._code_editor_line_number, this._code_editor_line_position + 1));
                        this._code_editor_line_position += current_potential_lexeme.length;
                        this.advance_last_position();
                    } // else
                } // else-if
                // Not in comment nor in a string
                    ...
                    if (/=$|\{$|\}$|\($|\)$|\"$|\+$|\/\*$|\*\/$|\s$|\$$/.test(current_potential_lexeme) && current_potential_lexeme.length > 1) {
                        ...
                        */
                            ...
                            let invalidToken = new NightingaleCompiler.LexicalToken(INVALID_TOKEN, null, current_potential_lexeme, this._code_editor_line_number, this._code_editor_line_position + current_potential_lexeme.length - (current_potential_lexeme.length - 1));
                     ...
                     */
                        ...
                        if (token.name.includes("SPACE_TAB")) {
                            this.emit_token_to_stream(token.copyWith(current_potential_lexeme, this._code_editor_line_number, this._code_editor_line_position));
                            this.advance_last_position();
                            this._code_editor_line_position += CODE_EDITOR_TAB_LENGTH;
                        } // if
                        else {
                            this._temp_token = token.copyWith(current_potential_lexeme, this._code_editor_line_number, this._code_editor_line_position + current_potential_lexeme.length - (current_potential_lexeme.length - 1));
                            this._current_position++;
                        } // else
                        /**
                         ...
                         */
                        // this.stacktrace_stack.push(this._temp_token);
            ...
            this.output.push(new NightingaleCompiler.OutputConsoleMessage(LEXER, INFO, `Lexer completed with ${this.warnings_stream.length} warnings.`));
            this.output.push(new NightingaleCompiler.OutputConsoleMessage(LEXER, INFO, `Lexer completed with ${this.errors_stream.length} errors.`));
            return sourceCode;
        } // function: main
        ...
        post_check_for_warnings(newSourceCode) {
            ...
            if (this.missingEndOfProgram) {
                this.output.push(new NightingaleCompiler.OutputConsoleMessage(LEXER, WARNING, `Missing EOP. Try adding $ to line ${this._code_editor_line_number}`));
        ...
        post_check_for_errors() {
            ...
            if (this.isInComment) {
                this.output.push(new NightingaleCompiler.OutputConsoleMessage(LEXER, WARNING, `Missing End End Comment. Try adding */ to line ${this._code_editor_line_number}`));
                this.emit_token_to_stream(new NightingaleCompiler.LexicalToken(MISSING_TOKEN, null, "*/", this._code_editor_line_number, -1));
            } // if
            if (this.isInString) {
                this.output.push(new NightingaleCompiler.OutputConsoleMessage(LEXER, WARNING, `Missing End String Boundary Expression. Try adding \" to line ${this._code_editor_line_number}`));
                this.emit_token_to_stream(new NightingaleCompiler.LexicalToken(MISSING_TOKEN, null, "\"", this._code_editor_line_number, -1));
            } // if
        } // post_check_for_errors
        /**
        ...
        emit_token_to_stream(newToken) {
            if (newToken.name == WARNING_TOKEN) {
                ...
                this.output.push(new NightingaleCompiler.OutputConsoleMessage(LEXER, ERROR, `${this._code_editor_line_number}:${this._code_editor_line_position} Unrecognized Token: ${newToken.lexeme}`));
